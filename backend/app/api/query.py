...
Endpoints for querying repositories:
    ...
    POST /api/repos/{repo_id}/query/stream → Server-Sent Events stream
...
# ═══════════════════════════════════════════════════════════════
# POST /api/repos/{repo_id}/query/stream
# ═══════════════════════════════════════════════════════════════
...
async def query_repo_stream(
    ...
    """
    Ask a question about a repository and stream the response back.
    Requires repo status to be "ready".
    """
    fb = _get_firebase_service()
    registry = _get_pipeline_registry()
    ...
    async def generate():
        try:
            for event in pipeline.query_stream(body.query, config):
                yield f"data: {json.dumps(event)}\n\n"
        except LLMError as e:
            yield f"data: {json.dumps({'error': 'LLM_API_ERROR', 'message': str(e), 'done': True})}\n\n"
        except Exception as e:
            yield f"data: {json.dumps({'error': 'INTERNAL_ERROR', 'message': str(e), 'done': True})}\n\n"
